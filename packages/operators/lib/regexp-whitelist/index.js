...
module.exports = async ({ value, params }) => {
  ...
  assert(all.length || any.length, "regexp-whitelist requires `all` or `any`");

  assert(
    all.every(r => r instanceof RegExp),
    "regexp-whitelist `all` only supports regular expressions"
  );

  assert(
    any.every(r => r instanceof RegExp),
    "regexp-whitelist `any` only supports regular expressions"
  );

  assert(
    typeof value === "string",
    "regexp-whitelist called with a non-string value"
  );
